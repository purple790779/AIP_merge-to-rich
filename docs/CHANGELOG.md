...
- 업적 상세 화면에서 발생하던 sticky 헤더 / blur / opacity 조합 기반 레이어 충돌을 정리해, 하위 업적 카드가 반투명 배경처럼 겹쳐 보이거나 터치가 불안정하던 문제를 수정
- 업적 상세 리스트 스크롤 컨테이너와 카드 터치 반응을 안정화해 상세 화면 전 구간에서 읽기/스크롤/터치 동작이 일관되도록 보정
